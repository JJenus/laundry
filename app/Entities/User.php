...
class User extends Entity
{
    ...
    {
      $config = config("Auth");
      if($config->requireActivation===false) {
        return true;
      } 
    ...
    public function getPermissions()
    {
        ...
        {
            $this->attributes["permissions"] = (new PermissionModel())->getPermissionsForUser($this->id);
        }
    ...
    public function getRoles()
    {
        ...
        if (empty($this->attributes["roles"]))
        {
            ...
            {
                $this->attributes["roles"][$group['group_id']] = strtolower($group['name']);
            }
        }
